...
def run_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
    ...
    model.CCSStorageCapitalCost = Param(initialize=1000) #Investment cost per ton CO2 stored [Euro/tCO2] Hard-coded
    model.CCSTransportCapitalCost = Param(initialize=23) #Investment cost per unit of capacity [Euro/tCO2 per year] Hard-coded
    model.CCSStorageFixedOMCost = Param(initialize=15) #Fixed O&M cost poer ton CO2 [Euro/tCO2] Hard-coded
    model.CCSTransportFixedOMCost = Param(initialize=1.3) #Fixed O&M cost per unit of capacity [Euro/tCO2] Hard-coded
    model.CCSRemFrac = Param(model.Generator, default=0.0, mutable=True)  #Change in CCSRemFrac
    ...
    model.CCSStorageEff = Param(initialize=0.95) #Storage efficiency Hard-coded
    model.CCSStorageLifetime = Param(initialize=40) #Storage Lifetime [Ref: https://www.globalccsinstitute.com/archive/hub/publications/119816/costs-co2-storage-post-demonstration-ccs-eu.pdf] Hard-coded
    ...
    data.load(filename=tab_file_path + "/" + 'Generator_CO2RemFrac.tab', param=model.CCSRemFrac, format="table")
    ...
    def prepInvCost_rule(model):
    	...
    	for g in model.Generator:
    		for i in model.PeriodActive:
                 ...
                 if (('CCS',g) in model.GeneratorsOfTechnology or ('CCS_high',g) in model.GeneratorsOfTechnology):
                    costperperiod+=((model.CCSTransportCapitalCost+model.CCSTransportFixedOMCost+((model.WACC/(1-((1+model.WACC)**(-model.CCSStorageLifetime))))*model.CCSStorageCapitalCost+model.CCSStorageFixedOMCost)*model.CCSStorageEff )* \
                                    (3.6/model.genEfficiency[g,i])*(model.CCSRemFrac[g]*model.genCO2TypeFactor[g]))/1000  ##Check this
    ...
    def prepOperationalCostGen_rule(model):
    	...
    	for g in model.Generator:
    		for i in model.PeriodActive:
    			if (('CCS',g) in model.GeneratorsOfTechnology or ('CCS_high',g) in model.GeneratorsOfTechnology):
    				costperenergyunit=(3.6/model.genEfficiency[g,i])*(model.genFuelCost[g,i]+(1-model.CCSRemFrac[g])*model.genCO2TypeFactor[g]*model.CO2price[i])+ \
    				(3.6/model.genEfficiency[g,i])*(model.CCSRemFrac[g]*model.genCO2TypeFactor[g]*model.CCSCostTSVariable[i])+ \
    ...
    def shed_cost_rule(model,n,i):
        return model.operationalDiscountrate*model.nodeLostLoadCost[n,i]
    model.shed_cost=Expression(model.Node,model.PeriodActive,rule=shed_cost_rule)
    ...
    def gen_operational_cost_rule(model,g,i):
        return model.operationalDiscountrate*model.genMargCost[g,i]
    model.gen_operational_cost=Expression(model.Generator,model.PeriodActive,rule=gen_operational_cost_rule)

    def genInv_compenent_rule(model,i):
        return sum(model.operationalDiscountrate*model.genInvCost[g,i]* model.genInvCap[n,g,i] for (n,g) in model.GeneratorsOfNode)
    model.genInv_compenent=Expression(model.PeriodActive,rule=genInv_compenent_rule)

    def transmission_component_rule(model,i):
        return sum(model.operationalDiscountrate*model.transmissionInvCost[n1,n2,i]*model.transmisionInvCap[n1,n2,i] for (n1,n2) in model.BidirectionalArc)
    model.transmission_component=Expression(model.PeriodActive,rule=transmission_component_rule)

    def storagePW_component_rule(model,i):
        return sum(model.operationalDiscountrate*(model.storPWInvCost[b,i]*model.storPWInvCap[n,b,i]) for (n,b) in model.StoragesOfNode)
    model.storagePW_component=Expression(model.PeriodActive,rule=storagePW_component_rule)

    def storageEN_component_rule(model,i):
        return sum(model.operationalDiscountrate*(model.storENInvCost[b,i]*model.storENInvCap[n,b,i]) for (n,b) in model.StoragesOfNode)
    model.storageEN_component=Expression(model.PeriodActive,rule=storageEN_component_rule)
    ...
    if EMISSION_CAP:
    	def emission_cap_rule(model, i, w):
    	    return sum(model.seasScale[s]*(1-model.CCSRemFrac[g])*model.genCO2TypeFactor[g]*(3.6/model.genEfficiency[g,i])*model.genOperational[n,g,h,i,w] for (n,g) in model.GeneratorsOfNode for (s,h) in model.HoursOfSeason)/1000000 \
    	        - model.CO2cap[i] <= 0   #
    ...
    f = open(result_file_path + "/" + versionp + '_output_objective.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Objective function value:" + str(value(instance.Obj))])

    f = open(result_file_path + "/" + versionp + '_output_gen.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    f = open(result_file_path + "/" + versionp + '_output_stor.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    f = open(result_file_path + "/" + versionp + 'output_transmision.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    f = open(result_file_path + "/" + versionp + '_output_transmision_operational.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    f = open(result_file_path + "/" + versionp + '_output_Operational.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    f = open(result_file_path + "/" + versionp + '_output_EuropePlot.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    f = open(result_file_path + "/" + versionp + '_output_EuropeSummary.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    for i in instance.PeriodActive:
        for w in instance.Scenario:
            ...
            value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w]*(1-instance.CCSRemFrac[g])*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i]) for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason))]
            if EMISSION_CAP:
    ...
    #dual of genMaxProd_rule (model, n, g, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_genMaxProd.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season","Hour"] #,"dual_genMaxProd"
    for g in instance.Generator:
        my_string = str(g)+"_Euro_per_MWh"
        my_header.append(my_string)
    writer.writerow(my_header)
    for n in instance.Node:
        for i in instance.PeriodActive:
            for w in instance.Scenario:
                for (s,h) in instance.HoursOfSeason:
                    my_string=[n,inv_per[int(i-1)],w,s,h]
                    for g in instance.Generator:
                        if (n,g) in instance.GeneratorsOfNode and value(instance.genOperational[n,g,h,i,w])>0:
                            my_string.append(value(instance.dual[instance.maxGenProduction[n, g, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                        elif (n,g) not in instance.GeneratorsOfNode:
                            my_string.append("*")
                        else:
                            my_string.append(0)
                    writer.writerow(my_string)
    ...
    #dual of ramping_rule(model, n, g, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_ramping_rule.csv', 'w', newline='')
    writer = csv.writer(f)
    ...
    writer.writerow(my_header)
    for (s,h) in instance.HoursOfSeason:
        if h in instance.FirstHoursOfPeakSeason or h in instance.FirstHoursOfRegSeason:
            pass
        else:
            for n in instance.Node:
                for i in instance.PeriodActive:
                    for w in instance.Scenario: 
                        ...
                        for g in instance.ThermalGenerators:
                            if (n,g) in instance.GeneratorsOfNode:
                                my_string.append(value(instance.dual[instance.ramping[n, g, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                            else:
                                my_string.append("*")
                        writer.writerow(my_string)    
    f.close()
    #dual of storage_energy_balance_rule(model, n, b, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_storage_energy_balance.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season","Hour"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MWh"
        my_header.append(my_string)
    writer.writerow(my_header)
    for n in instance.Node:
        for i in instance.PeriodActive:
                ...
                for (s,h) in instance.HoursOfSeason:
                    ...
                    for b in instance.Storage:
                        if (n,b) in instance.StoragesOfNode:
                            my_string.append(value(instance.dual[instance.storage_energy_balance[n, b, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                        else:
                            my_string.append("*")
                    writer.writerow(my_string)
    f.close()

    #dual of storage_seasonal_net_zero_balance_rule(model, n, b, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_storage_seasonal_net_zero_balance.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season","Hour"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MWh"
            ...
            for n in instance.Node:
                for i in instance.PeriodActive:
                    for w in instance.Scenario:
                        ...
                        for b in instance.Storage:
                            if (n,b) in instance.StoragesOfNode:
                                my_string.append(value(instance.dual[instance.storage_seasonal_net_zero_balance[n, b, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                            else:
                                my_string.append("*")
                        writer.writerow(my_string)
    f.close()
    
    #dual of storage_operational_cap_rule(model, n, b, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_storage_operational_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season","Hour"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MWh"
        ...
        for i in instance.PeriodActive:
            for w in instance.Scenario:
                for (s,h) in instance.HoursOfSeason:
                    my_string=[n,inv_per[int(i-1)],w,s,h]
                    for b in instance.Storage:
                        if (n,b) in instance.StoragesOfNode and value(instance.storOperational[n,b,h,i,w])>0:
                            my_string.append(value(instance.dual[instance.storage_operational_cap[n, b, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                        elif (n,b) not in instance.StoragesOfNode:
                            my_string.append("*")
                        else:
                            my_string.append(0)
                    writer.writerow(my_string)
    ...
    #daul of storage_power_discharg_cap_rule(model, n, b, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_storage_power_discharg_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season","Hour"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MWh"
        ...
        for i in instance.PeriodActive:
            for w in instance.Scenario:
                for (s,h) in instance.HoursOfSeason:
                    my_string=[n,inv_per[int(i-1)],w,s,h]
                    for b in instance.Storage:
                        if (n,b) in instance.StoragesOfNode and value(instance.storDischarge[n,b,h,i,w])>0:
                            my_string.append(value(instance.dual[instance.storage_power_discharg_cap[n, b, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                        elif (n,b) not in instance.StoragesOfNode:
                            my_string.append("*")
                        else:
                            my_string.append(0)
    ...
    #daul of storage_power_charg_cap_rule(model, n, b, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_storage_power_charg_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season","Hour"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MWh"
        ...
        for i in instance.PeriodActive:
            for w in instance.Scenario:
                for (s,h) in instance.HoursOfSeason:
                    my_string=[n,inv_per[int(i-1)],w,s,h]
                    for b in instance.Storage:
                        if (n,b) in instance.StoragesOfNode and value(instance.storCharge[n,b,h,i,w])>0:
                            my_string.append(value(instance.dual[instance.storage_power_charg_cap[n, b, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                        elif (n,b) not in instance.StoragesOfNode:
                            my_string.append('*')
                        else:
                            my_string.append(0)
    ...
    #dual of hydro_gen_limit_rule(model, n, g, s, i, w) [euro_per_MW]
    f = open(result_file_path + "/" + versionp + '_dual_hydro_gen_limit.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period","Scenario","Season"] 
    for g in instance.RegHydroGenerator:
        my_string = str(g)+"_Euro_per_MW"
        my_header.append(my_string)
    writer.writerow(my_header)
    for n in instance.Node:
        for i in instance.PeriodActive:
            for w in instance.Scenario:
                for s in instance.Season:
                    my_string=[n,inv_per[int(i-1)],w,s]
                    for g in instance.RegHydroGenerator:
                        if (n,g) in instance.GeneratorsOfNode:
                            my_string.append(value(instance.dual[instance.hydro_gen_limit[n, g, s, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w])))
                        else:
                            my_string.append("*")
                    writer.writerow(my_string)
    f.close()

    #dual of hydro_node_limit_rule(model, n,i) [euro_per_MW]
    f = open(result_file_path + "/" + versionp + '_dual_hydro_node_limit.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","dual_hydro_node_limit_euro_per_MW"])
    for n in instance.Node:
        for i in instance.PeriodActive:
            writer.writerow([n,inv_per[int(i-1)],
                            value(instance.dual[instance.hydro_node_limit[n, i]]/(instance.operationalDiscountrate))]) 
    f.close()
                            
    #dual of transmission_cap_rule(model, n1, n2, h, i, w) [euro_per_MWh]
    f = open(result_file_path + "/" + versionp + '_dual_transmission_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["FromNode","ToNode","Period","Scenario","Season","Hour","dual_transmission_cap_euro_per_MWh"])
    for (n1,n2) in instance.DirectionalLink:
        for i in instance.PeriodActive:
            for (s,h) in instance.HoursOfSeason:
                for w in instance.Scenario:
                    writer.writerow([n1,n2,inv_per[int(i-1)],s,w,h,
                                   value(instance.dual[instance.transmission_cap[n1, n2, h, i, w]]/(instance.operationalDiscountrate*instance.seasScale[s]*instance.sceProbab[w]))]) 
    f.close()

    #dual of lifetime_rule_gen(model, n, g, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_lifetime_rule_gen.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] #,"dual_genMaxProd"
    for g in instance.Generator:
        my_string = str(g)+"_Euro_per_MW"
        my_header.append(my_string)
    writer.writerow(my_header)
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for g in instance.Generator:
                if (n,g) in instance.GeneratorsOfNode:
                    my_string.append(value(instance.dual[instance.installedCapDefinitionGen[n, g, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close() 

    #dual of lifetime_rule_storEN(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_lifetime_rule_storEN.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
        my_header.append(my_string)
    writer.writerow(my_header)
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for b in instance.Storage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.installedCapDefinitionStorEN[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close()    

    #dual of lifetime_rule_storPOW(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_lifetime_rule_storPOW.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
    ...
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for b in instance.Storage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.installedCapDefinitionStorPOW[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close() 

    #dual of lifetime_rule_trans(model, n1, n2, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_lifetime_rule_trans.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["FromNode","ToNode","Period","dual_lifetime_rule_trans_euro_per_MW"])
    for (n1,n2) in instance.BidirectionalArc:
        for i in instance.PeriodActive:
            writer.writerow([n1,n2,inv_per[int(i-1)],
                            value(instance.dual[instance.installedCapDefinitionTrans[n1, n2, i]]/(instance.operationalDiscountrate))]) 
    f.close()

    #dual of investment_gen_cap_rule(model, t, n, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_investment_gen_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","Technology","dual_investment_gen_cap_euro_per_MW"])
    for n in instance.Node:
        for i in instance.PeriodActive:
            for t in instance.Technology:
                writer.writerow([n,inv_per[int(i-1)],t,
                            value(instance.dual[instance.investment_gen_cap[t, n, i]]/(instance.operationalDiscountrate))])
    f.close() 

    #dual of investment_trans_cap_rule(model, n1, n2, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_investment_trans_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["FromNode","ToNode","Period","dual_investment_trans_cap_euro_per_MW"])
    for (n1,n2) in instance.BidirectionalArc:
        for i in instance.PeriodActive:
            writer.writerow([n1,n2,inv_per[int(i-1)],
                            value(instance.dual[instance.investment_trans_cap[n1, n2, i]]/(instance.operationalDiscountrate))]) 
    f.close()

    #dual of investment_storage_power_cap_rule(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_investment_storage_power_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
    ...
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for b in instance.Storage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.investment_storage_power_cap[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close()

    #dual of investment_storage_energy_cap_rule(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_investment_storage_energy_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
    ...
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for b in instance.Storage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.investment_storage_energy_cap[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close()   

    #dual of installed_gen_cap_rule(model, t, n, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_installed_gen_cap_rule.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","Technology","dual_installed_gen_cap_rule_euro_per_MW"])
    for n in instance.Node:
        for i in instance.PeriodActive:
            for t in instance.Technology:
                writer.writerow([n,inv_per[int(i-1)],t,
                            value(instance.dual[instance.installed_gen_cap[t, n, i]]/(instance.operationalDiscountrate))])
    f.close() 

    #dual of installed_trans_cap_rule(model, n1, n2, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_installed_trans_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["FromNode","ToNode","Period","dual_investment_trans_cap_euro_per_MW"])
    for (n1,n2) in instance.BidirectionalArc:
        for i in instance.PeriodActive:
            writer.writerow([n1,n2,inv_per[int(i-1)],
                            value(instance.dual[instance.installed_trans_cap[n1, n2, i]]/(instance.operationalDiscountrate))]) 
    f.close()

    #dual of installed_storage_power_cap_rule(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_installed_storage_power_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
    ...
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for b in instance.Storage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.installed_storage_power_cap[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close()

    #dual of installed_storage_energy_cap_rule(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_installed_storage_energy_cap.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
    ...
    for n in instance.Node:
        for i in instance.PeriodActive:
            ...
            for b in instance.Storage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.installed_storage_energy_cap[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close() 

    #dual of power_energy_relate_rule(model, n, b, i) [euro_per_MW]
    f = open(result_file_path + "/" + 'results_output_dual_power_energy_relate.csv', 'w', newline='')
    writer = csv.writer(f)
    my_header = ["Node","Period"] 
    for b in instance.Storage:
        my_string = str(b)+"_Euro_per_MW"
    ...
    for n in instance.Node:
        for i in instance.PeriodActive:
            my_string=[n,inv_per[int(i-1)]]
            for b in instance.DependentStorage:
                if (n,b) in instance.StoragesOfNode:
                    my_string.append(value(instance.dual[instance.power_energy_relate[n, b, i]]/(instance.operationalDiscountrate)))
                else:
                    my_string.append("*")
            writer.writerow(my_string)
    f.close() 
    ...
    #loading to csv the parameter:SceProbab(sce)
    f = open(result_file_path + "/" + 'build_SceProbab.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Scenario","Probability"])
    for sce in instance.Scenario:
        writer.writerow([sce,value(instance.sceProbab[sce])])
    f.close()

    #loading to csv the parameter:genInvCost(g,i)
    f = open(result_file_path + "/" + 'build_genInvCost.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Generator","Period","genInvCost"])
    for g in instance.Generator:
        for i in instance.PeriodActive:
            writer.writerow([g,i,value(instance.genInvCost[g,i])])
    f.close()

    #loading to csv the parameter:storPWInvCost(b,i),storENInvCost[b,i]
    f = open(result_file_path + "/" + 'build_storInvCost.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["StorageType","Period","storPWInvCost","storENInvCost"])
    for b in instance.Storage:
        for i in instance.PeriodActive:
            writer.writerow([b,i,value(instance.storPWInvCost[b,i]),value(instance.storENInvCost[b,i])])
    f.close()

    #loading to csv the parameter:transmissionInvCost[n1,n2,i]
    f = open(result_file_path + "/" + 'build_transmissionInvCost.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["FromNode","ToNode","Period","transmissionInvCost"])
    for (n1,n2) in instance.BidirectionalArc:
    	for i in instance.PeriodActive:
            writer.writerow([n1,n2,i,value(instance.transmissionInvCost[n1,n2,i])])
    f.close()

    #loading to csv the parameter:genMargCost[g,i]
    f = open(result_file_path + "/" + 'build_genMargCost.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Generator","Period","genMargCost"])
    for g in instance.Generator:
        for i in instance.PeriodActive:
            writer.writerow([g,i,value(instance.genMargCost[g,i])])
    f.close()

    #loading to csv the parameter:genInitCap[n,g,i]
    f = open(result_file_path + "/" + 'build_genInitCap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Generator","genInitCap"])
    for (n,g) in instance.GeneratorsOfNode:
        for i in instance.PeriodActive:
            writer.writerow([n,g,i,value(instance.genInitCap[n,g,i])])
    f.close()

    #loading to csv the parameter:transmissionMaxInstalledCap[n1,n2,i]
    f = open(result_file_path + "/" + 'build_transmissionMaxInstalledCap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["FromNode","ToNode","Period","transmissionMaxInstalledCap"])
    for (n1,n2) in instance.BidirectionalArc:
    	for i in instance.PeriodActive:
            writer.writerow([n1,n2,i,value(instance.transmissionMaxInstalledCap[n1,n2,i])])
    f.close()

    #loading to csv the parameter:operationalDiscountrate
    f = open(result_file_path + "/" + 'build_operationalDiscountrate.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["operationalDiscountrate"])
    writer.writerow([value(instance.operationalDiscountrate)])
    f.close()

    #loading to csv the parameter:genMaxInstalledCap[n,t,i]
    f = open(result_file_path + "/" + 'genMaxInstalledCap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Technology","Period","genMaxInstalledCap"])
    for t in instance.Technology:
        for n in instance.Node:
            for i in instance.PeriodActive:
                writer.writerow([n,t,i,value(instance.genMaxInstalledCap[n,t,i])])
    f.close()    

    #loading to csv the parameter:storENMaxInstalledCap[n,b,i],storPWMaxInstalledCap[n,b,i]
    f = open(result_file_path + "/" + 'build_storMaxInstalledCap.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","StorageType","Period","storENMaxInstalledCap","storPWMaxInstalledCap"])
    for (n,b) in instance.StoragesOfNode:
        for i in instance.PeriodActive:
            writer.writerow([n,b,i,value(instance.storENMaxInstalledCap[n,b,i]),value(instance.storPWMaxInstalledCap[n,b,i])])
    f.close()

    #loading to csv the parameter: maxRegHydroGen[n,i,s,sce]
    f = open(result_file_path + "/" + 'build_maxRegHydroGen.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","Scenario","Season","maxRegHydroGen"])
    for n in instance.Node:
        for s in instance.Season:
            for i in instance.PeriodActive:
                for sce in instance.Scenario:
                    writer.writerow([n,i,sce,s,value(instance.maxRegHydroGen[n,i,s,sce])])
    f.close()

    #loading to csv the parameter:model.genCapAvail[n,g,h,w,i] 
    f = open(result_file_path + "/" + 'build_genCapAvail.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","Scenario","Season","Hour","Generator","genCapAvail"])
    for (n,g) in instance.GeneratorsOfNode:
        for (s,h) in instance.HoursOfSeason:
            for w in instance.Scenario:
                for i in instance.PeriodActive:
                    if value(instance.genCapAvailTypeRaw[g]) == 0:
                        writer.writerow([n,i,w,s,h,g,value(instance.genCapAvail[n,g,h,w,i])])
    f.close() 

    #loading to csv the parameter:discount_multiplier(period)
    f = open(result_file_path + "/" + 'build_discount_multiplier.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Period","discount_multiplier"])
    for period in instance.PeriodActive:
        writer.writerow([period,value(instance.discount_multiplier[period])])
    f.close()    

    #loading to csv the parameter:shed_cost[n,i]
    f = open(result_file_path + "/" + 'build_shed_cost.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","shed_cost"])
    for i in instance.PeriodActive:
        for n in instance.Node:
            writer.writerow([n,i,value(instance.shed_cost[n,i])])
    f.close()     

    #loading to csv the parameter:gen_operational_cost[g,i]
    f = open(result_file_path + "/" + 'build_gen_operational_cost.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Generator","Period","gen_operational_cost"])
    for i in instance.PeriodActive:
        for g in instance.Generator:
            writer.writerow([g,i,value(instance.gen_operational_cost[g,i])])
    f.close() 
    ...
    f = open(result_file_path + "/" + versionp + '_output_ObjectiveFunction_components.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Period","Generation_Cost","Transmission_Cost","Storage_PW_Cost","Storage_EN_Cost","loadshed_Cost","Operational_Cost"])
    for i in instance.PeriodActive:
        writer.writerow([i,value(instance.genInv_compenent[i]),value(instance.transmission_component[i]),value(instance.storagePW_component[i]),value(instance.storageEN_component[i]),value(instance.shedcomponent[i]),value(instance.operationalcost[i])])
    f.close() 
    ...
    f = open(result_file_path + "/" + versionp + '_output_Share.csv', 'w', newline='')
    writer = csv.writer(f)
    writer.writerow(["Node","Period","Scenario","Season","Hour","Generation_MW","Discharge","Charge","Import","Export","Loadshed","Emission"])
    for n in instance.Node:
        for i in instance.PeriodActive:
            for w in instance.Scenario:
                for (s,h) in instance.HoursOfSeason:
                    ...
                    value(sum(instance.genOperational[n,g,h,i,w] for g in instance.Generator if (n,g) in instance.GeneratorsOfNode)),
                    value(sum(instance.storageDischargeEff[b]*instance.storDischarge[n,b,h,i,w] for b in instance.Storage if (n,b) in instance.StoragesOfNode)),   
                    value(sum(instance.storCharge[n,b,h,i,w] for b in instance.Storage if (n,b) in instance.StoragesOfNode)),
                    value(sum(instance.lineEfficiency[link,n]*instance.transmisionOperational[link,n,h,i,w] for link in instance.NodesLinked[n])),
                    value(sum(instance.transmisionOperational[n,link,h,i,w] for link in instance.NodesLinked[n])),
                    value(instance.loadShed[n,h,i,w]),
                    value(sum((1-instance.CCSRemFrac[g])*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i])*instance.genOperational[n,g,h,i,w] for g in instance.Generator if (n,g) in instance.GeneratorsOfNode))])
    f.close()
    ...
    if IAMC_PRINT:
        ...
        for w in instance.Scenario:
            f = row_write(f, "Europe", "Emissions|CO2|Energy|Supply|Electricity", "Mt CO2/yr", "Year", [value(Mtonperton*sum(instance.seasScale[s]*(1-instance.CCSRemFrac[g])*instance.genCO2TypeFactor[g]*(GJperMWh/instance.genEfficiency[g,i])* \
        ...
        for g in instance.Generator:
            ...
            if value(instance.genCO2TypeFactor[g]) != 0:
                f = row_write(f, "Europe", "CO2 Emmissions|Electricity|"+dict_generators[str(g)], "tons/MWh", "Year", [value((1-instance.CCSRemFrac[g])*instance.genCO2TypeFactor[g]*(GJperMWh/instance.genEfficiency[g,i])) for i in instance.PeriodActive]) #CO2 factor per generator type
        for (n,g) in instance.GeneratorsOfNode:
